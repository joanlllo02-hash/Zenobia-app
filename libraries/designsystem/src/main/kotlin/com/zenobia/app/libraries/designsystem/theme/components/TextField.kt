...
fun TextField(
    ...
    BasicTextField(
        ...
        minLines = minLines,
        readOnly = readOnly,
        cursorBrush = SolidColor(ZenobiaTheme.colors.textPrimary),
    ...
    Column {
        ...
        TextFieldContainer(
            ...
            Row(modifier = Modifier.padding(16.dp)) {
                if (leadingIcon != null) {
                    CompositionLocalProvider(LocalContentColor provides ZenobiaTheme.colors.iconSecondary) {
                        leadingIcon()
                    }
    ...
    Surface(
        ...
        border = if (readOnly) {
            ...
            BorderStroke(
                ...
                color = when {
                    !enabled -> ZenobiaTheme.colors.borderDisabled
                    isError -> ZenobiaTheme.colors.borderCriticalPrimary
                    isFocused -> ZenobiaTheme.colors.borderInteractiveHovered
                    else -> ZenobiaTheme.colors.borderInteractiveSecondary
                }
            )
        },
        color = when {
            readOnly -> ZenobiaTheme.colors.bgSubtleSecondary
            !enabled -> ZenobiaTheme.colors.bgCanvasDisabled
            else -> ZenobiaTheme.colors.bgCanvasDefault
        },
        content = content
    )
}
...
private fun SupportingTextLayout(validity: TextFieldValidity?, supportingText: String) {
    Row(horizontalArrangement = spacedBy(4.dp)) {
        ...
        Text(
            text = supportingText,
            color = when (validity) {
                TextFieldValidity.Invalid -> ZenobiaTheme.colors.textCriticalPrimary
                TextFieldValidity.Valid -> ZenobiaTheme.colors.textSuccessPrimary
                else -> ZenobiaTheme.colors.textSecondary
            },
...
private fun textFieldStyle(enabled: Boolean): TextStyle {
    return ZenobiaTheme.typography.fontBodyLgRegular.copy(
        color = if (enabled) {
...
@Composable
internal fun TextFieldsLightPreview() = ZenobiaPreviewLight { ContentToPreview() }

@Preview(group = PreviewGroup.TextFields, heightDp = 1000)
@Composable
internal fun TextFieldsDarkPreview() = ZenobiaPreviewDark { ContentToPreview() }
